$(document).ready(function () { //On page load...
    ...
    $("#searchBtn").click(function () { //When Save button is clicked ... 
        ...
        const capitals = citySearched.charAt(0).toUpperCase() + citySearched.slice(1); //Make first word uppercase
        ...
        if ($("#searcher").val() == '') { //See if #searcher val is empty string (no input)
    ...
    errors(); //Calling this in document ready so it'll work on page load 

    function errors() {
        if ($(document).ajaxError(function () { //Error message checks/popups
                emptyCards();
...
function firstCall(city) {
    ...
    if (city) { //Check HERE if city (parameter) is existing, and if so...
        let inputs = city; //Make whatever input (searcher.val) is equal city
    } //Doing this here so we can save the most recent city searched
    ...
    $.ajax({
        url: newQuery,
        method: "GET",
    }).then(function (response) { //response returns full object of the url
        emptyCards();
        if ($(document).ajaxError(function () { //Error message checks/popups
            emptyCards();
            $("#mainstate").append("Error: " + $("#searcher").val() + " doesn't exist. Please search for a valid city.");
...
function twoCall(response) { //Response in the parameter because we need specifics
    ...
    $.ajax({
        ...
        buttonMaker(); //Calling here because we made the error statement in the firstCall() and if it meets the error requirements, it won't allow the button to be
        function moreDisplays() { //Don't put this function outside, only being called here
...
//Create new localStorage item that saves each city searched into an array and creates corresponding buttons (search history)
